...
def create_input_files(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
    ...
    for img in data['images']:
        ...
        if img['split'] in {'train', 'restval'}:#restval：除了训练其他都用来验证
    ...
    for impaths, imcaps, split in [(train_image_paths, train_image_captions, 'TRAIN'),
        ...
        with h5py.File(os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'), 'a') as h:
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                # Sample captions
                # 如果第i个图像的caption数量小于captions_per_image，则从imcaps[i]中随机选择captions_per_image - len(imcaps[i])个caption，并添加到imcaps[i]中
                if len(imcaps[i]) < captions_per_image:
                    captions = imcaps[i] + [choice(imcaps[i]) for _ in range(captions_per_image - len(imcaps[i]))]
                # 否则，从imcaps[i]中随机选择k个caption
                else:
                ...
                if dataset =='LEVIR_CC':
                    ...
                    if len(img_A.shape) == 2:#如果图像是灰度图（即只有二维），则将其转换为三维，并在第三个维度上复制三次，使其变成RGB图像。
                        img_A = img_A[:, :, np.newaxis]
                        img_A = np.concatenate([img_A, img_A, img_A], axis=2)
                    if len(img_B.shape) == 2:
                        img_B = img_B[:, :, np.newaxis]
                        img_B = np.concatenate([img_B, img_B, img_B], axis=2)
                    img_A = imresize(img_A, (256, 256))#插值方法：imresize默认使用双线性插值，来调整图像大小
                    ...
                    # Save image to HDF5 file
                    images[i] = img#将处理后的图像数据保存到images列表中

                for j, c in enumerate(captions):#captions列表存储的是图像的描述（caption）。每个描述是一个字符串，表示对图像内容的描述。例如，如果图像是一张猫的照片，那么描述可能类似于 "a cat sitting on a mat"。
                    # Encode captions，这段Python代码的主要功能是对给定的字幕（captions）进行编码，以便后续处理。具体来说，它将每个字幕转换为一个整数列表，其中每个整数代表一个单词的索引。
...
class AverageMeter(object):#用于跟踪某个指标的最新值、平均值、总和以及计数。这个类通常用于机器学习和深度学习中的性能评估，特别是在计算损失函数或准确率等指标时。
    """
